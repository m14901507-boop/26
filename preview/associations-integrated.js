(()=>{
  ...
  const style=document.createElement('style');style.textContent=`
    .association-hostbar{display:none;grid-template-columns:minmax(190px,1.25fr) repeat(6,minmax(120px,.8fr)) minmax(190px,1fr);gap:9px;align-items:stretch;margin:0 0 14px;padding:12px;border:1px solid #294059;border-radius:17px;background:linear-gradient(145deg,#101923,#091019)}
    .association-hostbar.active{display:grid}.assoc-host-cell{min-height:58px;padding:9px 11px;border:1px solid rgba(255,255,255,.07);border-radius:12px;background:rgba(255,255,255,.018)}.assoc-host-cell span{display:block;color:#8296aa;font-size:9px}.assoc-host-cell b{display:block;margin-top:6px;font-size:13px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.assoc-host-select{width:100%;margin-top:5px;border:0;background:transparent;color:#fff;font-weight:800;outline:none}.assoc-host-select option{background:#101923}.assoc-host-actions{display:grid;grid-template-columns:1fr 1fr;gap:7px}.assoc-host-action{min-height:28px;border:1px solid #30445c;border-radius:10px;background:#0c141e;color:#fff;font-weight:800;cursor:pointer;padding:5px 8px}.assoc-host-action.primary{background:linear-gradient(135deg,#168df2,#0866cc);border-color:#168df2}.assoc-host-action.green{border-color:#27d391;color:#74edbe}.assoc-host-action.red{border-color:#7b3040;color:#ff9aa6}.assoc-host-action:hover{transform:translateY(-1px)}
    @media(max-width:1350px){.association-hostbar{grid-template-columns:repeat(4,1fr)}.assoc-host-actions{grid-column:span 2}}@media(max-width:700px){.association-hostbar{grid-template-columns:1fr 1fr}.assoc-host-actions{grid-column:span 2}}@media(max-width:460px){.association-hostbar{grid-template-columns:1fr}.assoc-host-actions{grid-column:auto}}
  ...
  const host=document.createElement('div');host.className='association-hostbar';host.id='associationHostBar';host.innerHTML=`
    ...
    <div class="assoc-host-actions">
      <button id="hostAssocAdd" class="assoc-host-action primary" type="button">+ جمعية</button>
      <button id="hostMemberAdd" class="assoc-host-action green" type="button">+ عضو</button>
      <button id="hostAssocEdit" class="assoc-host-action" type="button">تعديل الجمعية</button>
      <button id="hostAssocDelete" class="assoc-host-action red" type="button">حذف الجمعية</button>
    </div>`;
  ...
  if(!page){
    ...
    page.innerHTML='<div style="border:1px solid #27394d;border-radius:18px;overflow:hidden;background:#05070a;min-height:1180px"><iframe id="associationsProFrame" title="إدارة الجمعيات" src="associations.html?v=10" style="width:100%;height:1180px;border:0;display:block;background:#05070a"></iframe></div>';
  ...
  document.getElementById('hostAssocAdd')?.addEventListener('click',()=>{try{childDoc()?.getElementById('newAssociation')?.click();}catch(e){}});
  document.getElementById('hostMemberAdd')?.addEventListener('click',()=>{try{syncChildSelection();const w=frame?.contentWindow;if(typeof w?.__floosyAssociationOpenMember==='function')w.__floosyAssociationOpenMember();else childDoc()?.getElementById('addMember')?.click();}catch(e){}});
  document.getElementById('hostAssocEdit')?.addEventListener('click',()=>{try{syncChildSelection();const w=frame?.contentWindow;if(typeof w?.__floosyAssociationEdit==='function')w.__floosyAssociationEdit();else{const d=childDoc()?.getElementById('associationForm')?.closest('details');if(d)d.open=true;}}catch(e){}});
