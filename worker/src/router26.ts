...
async function read(env:Env,t:string,name:string,range:string){
  const rg=encodeURIComponent(`'${name}'!${range}`);
  const d:any=await gj(`https://sheets.googleapis.com/v4/spreadsheets/${encodeURIComponent(env.SPREADSHEET_ID.trim())}/values/${rg}?valueRenderOption=UNFORMATTED_VALUE&dateTimeRenderOption=FORMATTED_STRING`,t);
  const v:Row[]=d.values||[];
  return{headers:v[0]||[],rows:v.slice(1)};
}

function activeGuide(g:Row){
  const name=txt(g[0]),label=txt(g[6]),active=txt(g[5]).toLowerCase(),all=[g[1],g[2],g[3],g[4]].map(txt).join(' ');
  if(!name||!label)return false;
  if(/^(لا|no|false|0|غير نشط)$/.test(active))return false;
...
async function listOne(t:string,label:string,pageToken:string){
  const q=new URLSearchParams({labelIds:label,maxResults:'1'});if(pageToken)q.set('pageToken',pageToken);
  const d:any=await gj(`https://gmail.googleapis.com/gmail/v1/users/me/messages?${q.toString()}`,t);
  return{id:txt(d.messages?.[0]?.id),nextPageToken:txt(d.nextPageToken)};
}
...
async function findOperationRow(env:Env,t:string,id:string){
  const col=await read(env,t,'العمليات','A:A');
  for(let i=0;i<col.rows.length;i++)if(txt(col.rows[i]?.[0])===id)return i+2;
  return 0;
}
async function readOperationRow(env:Env,t:string,row:number){const r=await read(env,t,'العمليات',`A${row}:T${row}`);return r.rows[0]||[];}
async function writeOperationRow(env:Env,t:string,row:number,values:Row){
  const rg=encodeURIComponent(`'العمليات'!A${row}:T${row}`);
  await gj(`https://sheets.googleapis.com/v4/spreadsheets/${encodeURIComponent(env.SPREADSHEET_ID.trim())}/values/${rg}?valueInputOption=USER_ENTERED`,t,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({majorDimension:'ROWS',values:[values]})});
}
async function appendOperationRow(env:Env,t:string,values:Row){
  const rg=encodeURIComponent("'العمليات'!A:T");
  await gj(`https://sheets.googleapis.com/v4/spreadsheets/${encodeURIComponent(env.SPREADSHEET_ID.trim())}/values/${rg}:append?valueInputOption=USER_ENTERED&insertDataOption=INSERT_ROWS`,t,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({majorDimension:'ROWS',values:[values]})});
}

async function previewSafeStep(env:Env,labelIndex:number,pageToken:string){
  const t=await token(env);
  const guide=await read(env,t,'دليل البنود','A:L');
  const valid=guide.rows.filter(activeGuide);
  if(!valid.length)return{ok:true,mode:'preview-safe-step',labels:0,labelIndex:0,scanned:0,added:0,updated:0,kept:0,ambiguous:0,changed:0,done:true,nextLabelIndex:null,nextPageToken:'',historyId:''};
  ...
  const page=await listOne(t,currentLabel,pageToken);
  let scanned=0,added=0,updated=0,kept=0,ambiguous=0;

  if(page.id){
    scanned=1;
    const m=await getMsg(t,page.id);
    const byLabel=new Map<string,Row>();valid.forEach(g=>byLabel.set(txt(g[6]),g));
    const matches=(m.labelIds||[]).map(l=>byLabel.get(txt(l))).filter(Boolean) as Row[];
    if(matches.length===1){
      const g=matches[0];
      const row=await findOperationRow(env,t,page.id);
      if(row){
        const old=await readOperationRow(env,t,row),next=applyGuide(old,g);
        if(JSON.stringify(next)!==JSON.stringify(old.slice(0,20))){await writeOperationRow(env,t,row,next);updated=1;}else kept=1;
      }else{await appendOperationRow(env,t,buildNew(g,m));added=1;}
    }else if(matches.length>1)ambiguous=1;
  }

  let nextLabelIndex=i,nextPageToken=page.nextPageToken;
  if(!nextPageToken)nextLabelIndex=i+1;
  ...
  return{ok:true,mode:'preview-safe-step',historyId,labels:valid.length,labelIndex:i,labelName:txt(currentGuide[0]),scanned,added,updated,kept,ambiguous,changed:added+updated,done,nextLabelIndex:done?null:nextLabelIndex,nextPageToken:done?'':nextPageToken};
}

export default{async fetch(request:Request,env:Env):Promise<Response>{
  const u=new URL(request.url);
  if(u.pathname==='/api/preview/sync/full'&&request.method==='OPTIONS')return new Response(null,{status:204,headers:cors(env)});
  if(u.pathname==='/api/preview/sync/full'&&request.method==='POST'){
    try{
      if(!(await validSession(request,env)))return json({ok:false,error:'Unauthorized'},env,401);
      const labelIndex=Math.max(0,Number(u.searchParams.get('labelIndex')||0)||0),pageToken=txt(u.searchParams.get('pageToken'));
      return json(await previewSafeStep(env,labelIndex,pageToken),env);
