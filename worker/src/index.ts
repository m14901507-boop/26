...
function extractAccountNumber(text:string,bankName=''){const candidates:string[]=[];for(const p of[/(?:account|acct|a\/?c)(?:\s*(?:no|number|ending|ending\s+in))?[^0-9#*xX]{0,20}[#*xX-]*(\d{2,4})\b/ig,/[#*xX]{2,}(\d{2,4})\b/g,/\b\d{2,5}[#*xX-]{2,}(\d{2,4})\b/g]){let m:RegExpExecArray|null;while((m=p.exec(text))!==null)if(m[1])candidates.push(m[1]);}let no=candidates[0]||'';if(bankName==='ميثاق'&&no==='21')no='0021';return no;}
