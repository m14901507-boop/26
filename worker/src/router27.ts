...
async function readSyncContext(env:Env,t:string){
  const base=`https://sheets.googleapis.com/v4/spreadsheets/${encodeURIComponent(env.SPREADSHEET_ID.trim())}/values:batchGet`;
  const qs=new URLSearchParams();
  qs.append('ranges',"'دليل البنود'!A:L");
  qs.append('ranges',"'العمليات'!A:T");
  qs.set('valueRenderOption','UNFORMATTED_VALUE');
  qs.set('dateTimeRenderOption','FORMATTED_STRING');
  const d:any=await gj(`${base}?${qs.toString()}`,t);
  const guideValues:Row[]=d.valueRanges?.[0]?.values||[];
  const opValues:Row[]=d.valueRanges?.[1]?.values||[];
  return{guideRows:guideValues.slice(1),opRows:opValues.slice(1)};
}
...
async function updateRows(env:Env,t:string,changes:Array<{row:number;values:Row}>){
  ...
  await gj(`https://sheets.googleapis.com/v4/spreadsheets/${encodeURIComponent(env.SPREADSHEET_ID.trim())}/values:batchUpdate`,t,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({valueInputOption:'USER_ENTERED',data})});
}
async function appendRows(env:Env,t:string,rows:Row[]){
  if(!rows.length)return;
  const rg=encodeURIComponent("'العمليات'!A:T");
  await gj(`https://sheets.googleapis.com/v4/spreadsheets/${encodeURIComponent(env.SPREADSHEET_ID.trim())}/values/${rg}:append?valueInputOption=USER_ENTERED&insertDataOption=INSERT_ROWS`,t,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({majorDimension:'ROWS',values:rows})});
}
...
async function previewQuotaSafeBatch(env:Env,labelIndex:number,pageToken:string){
  ...
  const existing=new Map<string,{row:number;values:Row}>();ctx.opRows.forEach((r,idx)=>{const id=txt(r[0]);if(id)existing.set(id,{row:idx+2,values:r});});
  ...
  await appendRows(env,t,newRows);
...
export default{async fetch(request:Request,env:Env):Promise<Response>{
  ...
  if(u.pathname==='/api/preview/sync/full'&&request.method==='POST'){
    try{
      ...
      return json(await previewQuotaSafeBatch(env,labelIndex,pageToken),env);
