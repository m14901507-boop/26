...
function isInternal(r:Row){const s=[r[3],r[6],r[7],r[9],r[16]].map(txt).join(' ');return /تحويل\s*داخلي|تحويلات\s*داخلية|internal\s*transfer/i.test(s);}
function isVisibleFinancial(r:Row){const id=txt(r[0]),item=txt(r[2]),movement=txt(r[6]),classification=txt(r[3]);if(!id||!item)return false;if(isInternal(r))return false;return !!(movement||classification);}
export default{async fetch(request:Request,env:Env):Promise<Response>{const u=new URL(request.url);if(request.method==='OPTIONS')return current.fetch(request,env);try{
 if(u.pathname==='/api/operations'&&request.method==='GET'){
   ...
   const ops=await readOps(env);const rows=ops.rows.filter(isVisibleFinancial);
